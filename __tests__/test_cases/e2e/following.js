...
describe('Given authenticated users, user A and B', () => {
  ...
  describe("When user A follows user B", () => {
    ...
    it("User A should see following as true when viewing user B's profile", async () => {
      const getProfileResponse = await when.a_user_calls_getProfile(userA, userBsProfile.screenName)
      // console.log(getProfileResponse)
      const following = getProfileResponse.following;
      const followedBy = getProfileResponse.followedBy
